...
def get_answer_and_validate():
    ...
    if random_response.status_code == 200:
        question_id1 : int= random_response.json()["id"]
        print("Random question retrieved successfully: ", question_id1,random_response.json()["question"])
    ...
    if random_response.status_code == 200:
        question_id2: int = random_response.json()["id"]
        print("Random question retrieved successfully: ", question_id2,random_response.json()["question"])
        answer_response = client.post("/answers/", params={
    ...
    answers_response = client.get(f"/questions/{question_id1}/answers")
    ...
    rating_response = client.post("/validation/rating", params={
        ...
        "flag_ia": False}, headers=headers2)
    assert rating_response.status_code == 201
    
    answers_response = client.get(f"/questions/{question_id2}/answers")
    assert answers_response.status_code in (200, 404)
    if answers_response.status_code == 200:
        answers = answers_response.json()
        answer_id2 = answers[0]["id"]
        assert isinstance(answers, list)
    ...
    rating_response = client.post("/validation/rating", params={
        ...
        "flag_ia": False}, headers=headers1)
    assert rating_response.status_code == 201

    validations_response = client.get(f"/answers/{answer_id1}/validations")
    assert validations_response.status_code in (200)
    ...
    validations_response = client.get(f"/answers/{answer_id2}/validations")
    assert validations_response.status_code in (200)
    if validations_response.status_code == 200:
        validations = validations_response.json()
        assert isinstance(validations, list)
